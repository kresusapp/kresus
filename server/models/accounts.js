...
export class AccountModel extends Model {
    ...
    get bank() {
        log.error('Trying to use undefined bank property!');
...
export default class Accounts {
    static async all(userId) {
        return await AccountModel.query().where({ userId });
    }

    static async byAccess(userId, access) {
        return await AccountModel.query().where({ userId, accessId: access.id });
    }

    static async create(userId, account) {
        assert(typeof account !== 'undefined' && account.customFields instanceof Array);
