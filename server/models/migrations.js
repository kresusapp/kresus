...
let migrations = [
  ...
  async function m4(cache) {
    log.info(
      "Migrating HelloBank users to BNP and BNP users to the new website format."
    );
  ...
  async function m5(cache) {
    ...
    for (let a of cache.accounts) {
      ...
      log.info(
        `\tImport date for ${a.title} (${a.accountNumber}): ${a.importDate}`
      );
    }
  },
      ...
      if (cache.types.length) {
        ...
        for (let operation of operations) {
          if (
            ...
            typeMap.has(operation.operationTypeID)
          ) {
      ...
      for (let access of accesses) {
        ...
        switch (website) {
          ...
          case "mobi.ere.hsbc.fr":
            ...
            log.error(
              "\tPlease create a new access using erehsbc module (HSBC ERE)."
            );
            break;
          default:
  ...
  async function m11(cache) {
    ...
    try {
      ...
      for (let account of cache.accounts.filter(acc => acc.iban === "None")) {
        log.info(
          `\tDeleting iban for ${account.title} of bank ${account.bank}`
        );
        delete account.iban;
        await account.save();
      }
...
export async function run() {
  const migrationVersion = await Config.findOrCreateDefault(
    "migration-version"
  );
