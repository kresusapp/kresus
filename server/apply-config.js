...
module.exports = function prepareProcessKresus(standalone, config) {
    ...
    process.kresus.emailTransport =
        process.env.KRESUS_EMAIL_TRANSPORT || (config && config.email && config.email.transport) || '';

    process.kresus.emailSendmailBin =
        process.env.KRESUS_EMAIL_SENDMAIL_BIN || (config && config.email && config.email.sendmail_bin) || '';
