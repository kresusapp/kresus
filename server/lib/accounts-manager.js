...
async function retrieveAllAccountsByAccess(access) {
  ...
  log.info(
    `Retrieve all accounts from access ${access.bank} with login ${access.login}`
  );
...
async function notifyNewOperations(access, newOperations, accountMap) {
  ...
  for (let [accountId, ops] of newOpsPerAccount.entries()) {
    ...
    if (ops.length === 1) {
      ...
      params.operation_details = `${ops[0].title} ${formatCurrency(
        ops[0].amount
      )}`;
    }
...
class AccountManager {
  ...
  async retrieveNewAccountsByAccess(access, shouldAddNewAccounts) {
    if (this.newAccountsMap.size) {
      log.warn(
        "At the top of retrieveNewAccountsByAccess, newAccountsMap must be empty."
      );
    ...
    for (let account of diff.providerOrphans) {
      ...
      if (existingOperations.length) {
        let offset = existingOperations.reduce(
...
- ${provided.accountNumber} / ${provided.title}`);
      ...
      log.info(`Found ${diff.duplicateCandidates
        .length} candidates for merging, but not
...
offset of ${balanceOffset}.`);
    ...
    if (numNewOperations > 0) {
      log.info(
        `Informing user ${numNewOperations} new operations have been imported...`
      );
  ...
  async resyncAccountBalance(account) {
    ...
    if (typeof retrievedAccount !== "undefined") {
      ...
      let operationsSum = operations.reduce(
        (amount, op) => amount + op.amount,
        0
      );
