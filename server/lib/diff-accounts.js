  ...
  for (let i = 0; i < knowns.length; i++) {
    ...
    for (let j = 0; j < provideds.length; j++) {
      ...
      let accountNumberScore =
        known.accountNumber === provided.accountNumber
          ? HEURISTICS.SAME_ACCOUNT_NUMBER
          : 0;
      let ibanScore = known.iban === provided.iban ? HEURISTICS.SAME_IBAN : 0;
      let currencyScore =
        known.currency === provided.currency ? HEURISTICS.SAME_CURRENCY : 0;

      scores[i][j] =
        titleScore + accountNumberScore + ibanScore + currencyScore;
    }
  }
