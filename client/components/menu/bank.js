...
class BankListItemComponent extends React.Component {
    ...
    render() {
        ...
        if (this.state.showAccounts) {
            accountsElements = this.props.access.accounts.map(id => (
                <AccountListItem
                    key={id}
                    accountId={id}
...
const Export = connect((state, props) => {
    let accountsIds = get.accountsByAccessId(state, props.accessId);

    let currency = null;
    let sameCurrency = true;
    let formatCurrency;
    let total = 0;
    for (let id of accountsIds) {
        let acc = get.accountById(state, id);
