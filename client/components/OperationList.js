...
class DetailedViewLabelComponent extends LabelComponent {
    constructor(props) {
        has(props, 'operation');
        props.defaultValue = () => {return defaultValue()};
        props.buttonLabel = () => {return buttonLabel()};        
        super(props);
    }
...
class OperationListViewLabelComponent extends LabelComponent {
    ...
    buttonLabel() {
        if (this.props.operation.customLabel === null || this.props.operation.customLabel.trim().length === 0){
            return <div className="label-button text-uppercase">{this.props.label}</div>;
        }
        return <div className="label-button text-uppercase">{this.props.operation.customLabel}</div>;
    }
