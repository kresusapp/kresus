...
class OperationsComponent extends React.Component {
  ...
  render() {
    ...
    let lastCheckedDate = formatDate.toShortString(
      this.props.account.lastChecked
    );
    ...
    let positiveSum = computeTotal(
      format,
      x => x.amount > 0,
      wellOperations,
      0
    );
    let negativeSum = computeTotal(
      format,
      x => x.amount < 0,
      wellOperations,
      0
...
function filter(operations, search) {
  ...
  filtered = filterIf(
    search.type !== "",
    filtered,
    op => op.type === search.type
  );

  filtered = filterIf(
    search.amountLow !== null,
    filtered,
    op => op.amount >= search.amountLow
  );

  filtered = filterIf(
    search.amountHigh !== null,
    filtered,
    op => op.amount <= search.amountHigh
  );

  filtered = filterIf(
    search.dateLow !== null,
    filtered,
    op => op.date >= search.dateLow
  );

  filtered = filterIf(
    search.dateHigh !== null,
    filtered,
    op => op.date <= search.dateHigh
  );
