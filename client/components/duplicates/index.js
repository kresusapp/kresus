...
function computePrevNextThreshold(current) {
  ...
  let nextValues = THRESHOLDS_SUITE.filter(
    v => v > Math.max(current, previousThreshold)
  );
...
export default connect(
  (state, props) => {
    ...
    let formatCurrency = get.accountById(state, currentAccountId)
      .formatCurrency;

    let duplicateThreshold = parseFloat(
      get.setting(state, "duplicateThreshold")
    );

    // Show the "more"/"fewer" button if there's a value after/before in the thresholds suite.
    let allowMore =
      duplicateThreshold <= THRESHOLDS_SUITE[NUM_THRESHOLDS_SUITE - 2];
    let allowFewer = duplicateThreshold >= THRESHOLDS_SUITE[1];

    let [prevThreshold, nextThreshold] = computePrevNextThreshold(
      duplicateThreshold
    );
