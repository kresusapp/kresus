...
class NewBankForm extends React.Component {
    ...
    handleSubmit(event) {
        ...
        let staticBank = this.selectedBank();

        let customFields = [];
        if (staticBank.customFields.length) {
            customFields = staticBank.customFields.map(field => {
    ...
    render() {
        ...
        if (staticBank.customFields.length > 0) {
            maybeCustomFields = staticBank.customFields.map(field => {
                return (
                    <CustomBankField
                      onChange={ handleCustomFieldChange }
                      name={ field.name }
                      bank={ staticBank.uuid }
                      key={ `${staticBank.uuid}-${field.name}` }
                    />
                );
            });
        }
