...
export const get = {
  ...
  initialAccountId(state) {
    ...
    if (
      defaultAccountId ===
      Settings.getDefaultSetting(state.settings, "defaultAccountId")
    ) {
  ...
  operationsByAccountIds(state, accountIds) {
    ...
    for (let accountId of accountIdsArray) {
      operations = operations.concat(
        Bank.operationsByAccountId(state.banks, accountId)
      );
    }
    return operations;
  },
  ...
  boolSetting(state, key) {
    ...
    assert(
      val === "true" || val === "false",
      "A bool setting must be true or false"
    );
    return val === "true";
  },
  ...
  isWeboobInstalled(state) {
    ...
    return (
      semver(version) &&
      semver.gte(version, normalizeVersion(MIN_WEBOOB_VERSION))
    );
  },
...
export const actions = {
  ...
  updateAccess(dispatch, accessId, login, password, customFields) {
    ...
    assert(
      ...
      "second param accessId must be a string"
    );
    ...
    if (typeof customFields !== "undefined") {
      assert(
        customFields instanceof Array &&
          customFields.every(
            f => assertHas(f, "name") && assertHas(f, "value")
          ),
