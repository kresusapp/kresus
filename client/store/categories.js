...
export function create(category) {
  assertHas(
    category,
    "title",
    "CreateCategory expects an object that has a title field"
  );
  assertHas(
    category,
    "color",
    "CreateCategory expects an object that has a color field"
  );
...
export function update(former, category) {
  assert(
    former instanceof Category,
    "UpdateCategory first arg must be a Category"
  );
  assertHas(
    category,
    "title",
    "UpdateCategory second arg must have a title field"
  );
  assertHas(
    category,
    "color",
    "UpdateCategory second arg must have a color field"
  );
...
export function destroy(category, replace) {
  assert(
    ...
    "DeleteCategory first arg must be a Category"
  );
  assert(
    typeof replace === "string",
    "DeleteCategory second arg must be a String id"
  );
...
export function initialState(categories) {
  ...
  let items = sortCategories(
    [NONE_CATEGORY].concat(categories).map(c => new Category(c))
  );
