...
export function removeAccount(state, accountId) {
    ...
    // Unset the defaultAccountId if we juste deleted it.
    ...
    newState = u.updateIn(
        'alerts',
        u.reject(alert => alert.bankAccount === account.accountNumber),
        newState
    );
...
export function addOperation(state, operation) {
    ...
    let operationIds = [operation.id].concat(account.operationIds);
    operationIds = sortOperationsById(newState, operationIds);

    return updateAccountInternal(newState, account.id, {
        operationIds,
...
function reduceMergeOperations(state, action) {
    ...
    if (status === SUCCESS) {
        ...
        return updateOperationInternal(ret, action.toKeep.id, newKept);
    }
...
function reduceResyncBalance(state, action) {
    ...
    if (status === SUCCESS) {
        ...
        let balance = initialAmount - account.initialAmount + account.balance;
    ...
    if (status === SUCCESS) {
        ...
        if (formerAccessStillExists) {
            ...
            if (otherAccessId) {
                ...
                currentAccountId = accountIdsByAccessId(ret, currentAccessId)[0];
            }
...
export function initialState(external, allAccesses, allAccounts, allOperations, allAlerts) {
    ...
    if (defaultAccountId && accountsMap[defaultAccountId]) {
        ...
        currentAccountId = accessesMap[currentAccessId].accountIds[0];
    }
...
export function alertPairsByType(state, alertType) {
    ...
    for (let alert of state.alerts.filter(a => a.type === alertType)) {
        let account = accountByNumber(state, alert.bankAccount);
        if (account === null) {
