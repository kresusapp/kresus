...
export function setOperationCategory(operation, categoryId) {
  assert(
    typeof operation.id === "string",
    "SetOperationCategory first arg must have an id"
  );
  assert(
    typeof categoryId === "string",
    "SetOperationCategory 2nd arg must be String id"
  );
  ...
  return dispatch => {
    dispatch(
      basic.setOperationCategory(operation, categoryId, formerCategoryId)
    );
    backend
      .setCategoryForOperation(operation.id, serverCategoryId)
      .then(() => {
        dispatch(
          success.setOperationCategory(operation, categoryId, formerCategoryId)
        );
      })
      .catch(err => {
        dispatch(
          fail.setOperationCategory(
            err,
            operation,
            categoryId,
            formerCategoryId
...
export function setOperationCustomLabel(operation, customLabel) {
  assert(
    typeof operation.id === "string",
    "setCustomLabel first arg must have an id"
  );
  assert(
    typeof customLabel === "string",
    "setCustomLabel 2nd arg must be String id"
  );
  ...
  return dispatch => {
    ...
    backend
      ...
      .catch(err => {
        dispatch(
          fail.setCustomLabel(err, operation, customLabel, formerCustomLabel)
        );
      });
  };
}
...
export function deleteAccess(accessId, get) {
  ...
  return (dispatch, getState) => {
    let accountsIds = get
      .accountsByAccessId(getState(), accessId)
      .map(acc => acc.id);
...
function handleFirstSyncError(err) {
  switch (err.code) {
    ...
    case Errors.INVALID_PARAMETERS:
      alert(
        $t("client.sync.invalid_parameters", { content: err.content || "?" })
      );
      break;
...
export function createAccess(get, uuid, login, password, fields) {
  return (dispatch, getState) => {
    ...
    backend
      ...
      .then(results => {
        dispatch(
          success.createAccess(uuid, login, password, fields, access, results)
        );
      })
      .catch(err => {
...
function finishSync(state, results) {
  ...
  if (typeof accounts !== "undefined") {
    ...
    accounts = accounts.map(
      a => new Account(a, state.constants.defaultCurrency)
    );
...
function reduceMergeOperations(state, action) {
  ...
  if (status === SUCCESS) {
    ...
    let ret = u.updateIn(
      "operations",
      u.reject(o => o.id === action.toRemove.id),
      state
    );
...
function reduceResyncBalance(state, action) {
  ...
  if (status === SUCCESS) {
    ...
    return u.updateIn(
      "accounts",
      updateMapIf("id", accountId, u({ initialAmount })),
      state
    );
  }
...
function reduceDeleteAccountInternal(state, accountId) {
  ...
  ret = u.updateIn(
    "operations",
    u.reject(o => o.bankAccount === accountNumber),
    ret
  );
  ...
  ret = u.updateIn(
    "alerts",
    u.reject(a => a.bankAccount === accountNumber),
    ret
  );
...
function reduceDeleteAccount(state, action) {
  ...
  if (status === SUCCESS) {
    ...
    let formerAccessStillExists = !!ret.accesses.filter(
      a => a.id === formerAccessId
    ).length;
    ...
    if (formerAccessStillExists) {
      ...
      currentAccountId = ret.accounts.filter(
        a => a.bankAccess === currentAccessId
      )[0].id;
    } else {
...
function reduceDeleteAccess(state, action) {
  ...
  if (status === SUCCESS) {
    ...
    if (getCurrentAccessId(state) === accessId) {
      ...
      let otherAccounts = ret.accounts.filter(
        a => a.bankAccess === currentAccessId
      );
...
function reduceUpdateAlert(state, action) {
  ...
  if (status === SUCCESS) {
    ...
    return u.updateIn(
      "alerts",
      updateMapIf("id", alertId, u(attributes)),
      state
    );
  }
...
function sortOperations(ops) {
  ...
  ops.sort((a, b) => {
    ...
    if (ad > bd) return -1;
    let ac =
      a.customLabel && a.customLabel.trim().length ? a.customLabel : a.title;
    let bc =
      b.customLabel && b.customLabel.trim().length ? b.customLabel : b.title;
...
export function operationById(state, operationId) {
  let candidates = state.operations.filter(
...
export function alertPairsByType(state, alertType) {
  ...
  for (let al of state.alerts.filter(a => a.type === alertType)) {
    let accounts = state.accounts.filter(
      acc => acc.accountNumber === al.bankAccount
    );
    if (!accounts.length) {
