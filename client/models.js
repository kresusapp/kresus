...
export class Account {
  constructor(arg, defaultCurrency) {
    assert(
      ...
      "defaultCurrency must be a string"
    );
...
export class Operation {
  constructor(arg) {
    ...
    this.attachments =
      (maybeHas(arg, "attachments") && arg.attachments) || null;
    this.raw = assertHas(arg, "raw") && arg.raw;
    this.dateImport =
      (maybeHas(arg, "dateImport") && new Date(arg.dateImport)) || 0;
    this.id = assertHas(arg, "id") && arg.id;
    this.categoryId = arg.categoryId || NONE_CATEGORY_ID;
    this.type = arg.type || UNKNOWN_OPERATION_TYPE;
    this.customLabel =
      (maybeHas(arg, "customLabel") && arg.customLabel) || null;
  }
}
...
export class Category {
  constructor(arg) {
    this.title = assertHas(arg, "title") && arg.title;
    this.color =
      (maybeHas(arg, "color") && arg.color) || stringToColor(this.title);
    let threshold = 0;
...
export class Alert {
  constructor(arg) {
    ...
    // Data for reports
    this.frequency =
      arg.type === "report" && assertHas(arg, "frequency") && arg.frequency;
    if (arg.type === "report")
