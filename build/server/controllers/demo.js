...
async function setupDemoMode(userId) {
    ...
    for (const category of default_categories_json_1.default) {
        if (existingCategories.has(helpers_1.translate(category.label))) {
            continue;
        }
    ...
    const response = await accesses_1.createAndRetrieveData(userId, {
        vendorId: 'demo',
        login: 'mylogin',
        password: 'couldnotcareless',
        customLabel: 'Demo bank',
    });
    helpers_1.assert(response.kind === 'value', "demo account shouldn't require a user action");
...
async function enable(req, res) {
    try {
        ...
        const isEnabled = await instance_1.isDemoEnabled(userId);
        if (isEnabled) {
        ...
        helpers_1.asyncErr(res, err, 'when enabling demo mode');
    }
}
...
async function disable(req, res) {
    try {
        ...
        helpers_1.asyncErr(res, err, 'when disabling demo mode');
    }
}
