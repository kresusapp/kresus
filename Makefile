...
docker-nightly-dev: docker-nightly-base ## Prepares for a Docker nightly (no need for make release).
	docker build -t bnjbvr/kresus-nightly-dev -f docker/Dockerfile-nightly-dev .

docker-nightly-prod: docker-nightly-base ## Prepares for a Docker nightly (no need for make release).
