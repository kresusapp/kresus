...
let { ofxToKresus, getAllData: exportData } = testing;
...
describe('import', () => {
    ...
    describe('duplicates pairs to ignore', () => {
        ...
        async function actualIgnoredPairs() {
            const all = await DuplicatesIgnored.all(USER_ID);
            return normalizePairs(all.map(p => [p.transactionId, p.otherTransactionId]));
        }

        // The amounts are unique in the world below, so they're a handy way to find a given
        // transaction, be it before or after the import.
        function idForAmount(transactions, amount) {
            const found = transactions.filter(tr => tr.amount === amount);
            assert.strictEqual(found.length, 1, `exactly one transaction of amount ${amount}`);
            return found[0].id;
        }

        // The world, with two extra transactions on the second account (which has none), so the
        // ignored pairs can span both accounts. Every transaction is given an id which can't
        // collide with the ones generated by the database, to make sure the pairs are actually
        // remapped and not blindly re-used.
        function newWorldWithTransactionIds() {
            const data = newWorld();
            ...
            data.transactions.forEach((tr, i) => {
                tr.id = 1000 + i;
            });
        ...
        // The transactions of both accounts, after an import.
        async function importedTransactionsByAccount() {
            const accounts = await Account.all(USER_ID);
            assert.strictEqual(accounts.length, 2);
            return [
                await Transaction.byAccount(USER_ID, accounts[0].id),
                await Transaction.byAccount(USER_ID, accounts[1].id),
            ];
        }

        it('should not fail on an export made before the ignored pairs were exported', async () => {
            await cleanAll(USER_ID);

            // No `duplicates` key at all, like every export made by an older Kresus.
            await importData(USER_ID, newWorldWithTransactionIds());

            assert.strictEqual((await Transaction.all(USER_ID)).length, 8 + 2);
            assert.deepStrictEqual(await actualIgnoredPairs(), []);
        });

        it('should not fail on an export with a duplicates object but no ignored pairs', async () => {
            await cleanAll(USER_ID);

            const data = newWorldWithTransactionIds();
            data.duplicates = {};
            await importData(USER_ID, data);
        ...
        it('should import the ignored pairs of every account, remapped to the new transaction ids', async () => {
            await cleanAll(USER_ID);

            const data = newWorldWithTransactionIds();
            data.duplicates = {
                ignored: [
                    [idForAmount(data.transactions, -83.8), idForAmount(data.transactions, -60.8)],
                    [idForAmount(data.transactions, -111), idForAmount(data.transactions, -222)],
                ],
            };

            await importData(USER_ID, data);

            const [first, second] = await importedTransactionsByAccount();
            assert.deepStrictEqual(
                await actualIgnoredPairs(),
                normalizePairs([
                    [idForAmount(first, -83.8), idForAmount(first, -60.8)],
                    [idForAmount(second, -111), idForAmount(second, -222)],
                ])
            );
        });

        it('should skip the pairs referencing transactions which were not part of the export', async () => {
            await cleanAll(USER_ID);
            ...
            data.duplicates = {
                ignored: [
                    [idForAmount(data.transactions, -83.8), idForAmount(data.transactions, -60.8)],
                    // 4242 was never part of the export.
                    [idForAmount(data.transactions, -111), 4242],
                ],
            };

            await importData(USER_ID, data);

            const [first] = await importedTransactionsByAccount();
            assert.deepStrictEqual(
                await actualIgnoredPairs(),
                normalizePairs([[idForAmount(first, -83.8), idForAmount(first, -60.8)]])
            );
        });

        it('should skip the pairs referencing transactions which were ignored on import', async () => {
            await cleanAll(USER_ID);

            const data = newWorldWithTransactionIds();

            // This one is invalid (no amount): it won't be imported, so the pair referencing it
            // must be discarded.
            const invalid = data.transactions.find(tr => tr.label === 'No amount');
            assert.ok(invalid, 'the world should contain a transaction without amount');
            assert.ok(typeof invalid.amount !== 'number');

            data.duplicates = {
                ignored: [
                    [idForAmount(data.transactions, -83.8), idForAmount(data.transactions, -60.8)],
                    [idForAmount(data.transactions, -111), invalid.id],
                ],
            };
        ...
        it('should not import the same pair twice', async () => {
            await cleanAll(USER_ID);

            const data = newWorldWithTransactionIds();
            const firstId = idForAmount(data.transactions, -83.8);
            const secondId = idForAmount(data.transactions, -60.8);
            data.duplicates = {
                ignored: [
                    [firstId, secondId],
                    // The same pair, in the reverse order.
                    [secondId, firstId],
                ],
            };
            ...
            assert.strictEqual((await DuplicatesIgnored.all(USER_ID)).length, 1);
        });

        it('should restore the very same ignored pairs after an export/import cycle', async () => {
            await cleanAll(USER_ID);
            await importData(USER_ID, newWorldWithTransactionIds());

            const [first, second] = await importedTransactionsByAccount();
            await DuplicatesIgnored.create(
                USER_ID,
                idForAmount(first, -83.8),
                idForAmount(first, -60.8)
            );
            await DuplicatesIgnored.create(
                USER_ID,
                idForAmount(second, -111),
                idForAmount(second, -222)
            );

            // Serialize the export the same way a real one is written to a file.
            const exported = JSON.parse(
                JSON.stringify(await exportData(USER_ID, { isExport: true }))
            );

            // Then re-import it in a clean database.
            await cleanAll(USER_ID);
            await importData(USER_ID, exported);

            const [newFirst, newSecond] = await importedTransactionsByAccount();
            assert.deepStrictEqual(
                await actualIgnoredPairs(),
                normalizePairs([
                    [idForAmount(newFirst, -83.8), idForAmount(newFirst, -60.8)],
                    [idForAmount(newSecond, -111), idForAmount(newSecond, -222)],
                ])
            );
        });
    });
