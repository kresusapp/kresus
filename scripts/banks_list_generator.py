...
def ignore_select_field(module_name, field_val):
    return (
        module_name == "banquepopulaire"
        and field_val in BANQUE_POPULAIRE_DEPRECATED_WEBSITES
    )
...
def format_kresus(backend, module, is_deprecated=False, module_loader=None):
    ...
    for key, value in config:
        ...
        if optional and key in IGNORE_FIELDS_LIST:
            print_error(
                'Skipping optional key "%s" for module "%s".' % (key, module.name)
            )
            continue
    ...
    def list_bank_modules(self):
        ...
        for module_name in sorted(self.modules_loader.iter_existing_module_names()):
            ...
            if "login" not in module.config and "username" not in module.config:
                print_error(
                    'Ignoring module "%s". It does not have login.' % module_name
                )
                continue

            if "password" not in module.config:
                print_error(
                    'Ignoring module "%s". It does not have password.' % module_name
                )
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="The file to write the output of the command.",
        default=None,
    )
    parser.add_argument(
        "-i",
        "--ignore-fakemodules",
        help="Don't add the fakemodules to the list (default: false)",
        default=False,
        action="store_true",
    )
    ...
    if options.ignore_fakemodules:
        ...
        fake_modules_path = os.path.abspath(
            os.path.join(
    ...
    data = json.dumps(
        content, ensure_ascii=False, indent=4, separators=(",", ": "), sort_keys=True
    ).encode("utf-8")
